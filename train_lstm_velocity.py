...
# function to calculate velocity from position
...
# Concatenate all datasets to compute the global scaler
all_data = []
for file in csv_files:
    file_path = os.path.join(dir_path, file)
    df = pd.read_csv(file_path, usecols=['timestamp', 'tx', 'ty', 'tz'])
    df['vx'] = calculate_velocity(df['tx'].values, df['timestamp'].values)
    df['vy'] = calculate_velocity(df['ty'].values, df['timestamp'].values)
    df['vz'] = calculate_velocity(df['tz'].values, df['timestamp'].values)

    # drop the rows with NaN values
    df = df.dropna()

    dataset = df[['vx', 'vy', 'vz']].values[1:].astype('float32')  # exclude the first row because np.diff reduces the length by 1
...
model_file = 'drone_velocity_prediction_model.h5'
model.save(model_file)
print(f"Model trained and saved at {model_file}")

# Save the scaler for future use
scaler_file = 'scaler_velocity.pkl'
